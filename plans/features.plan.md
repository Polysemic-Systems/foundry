...
19. [x] Cap the runner's captured stdout and stderr at a named byte limit, appending an explicit truncation marker that records how many bytes were dropped, so a runaway job cannot exhaust memory - files: crates/foundry-cli/src/runner.rs - run: cargo test -p foundry-cli
20. [x] Doctor warns when the evidence blob store contains orphaned objects older than the sweep age guard, reusing the graph's referenced-digest scan - files: crates/foundry-cli/src/main.rs, crates/foundry-core/src/graph.rs - run: cargo test
21. [x] Record truncation as structured evidence: RunnerOutput and JobResult carry truncated flags and dropped-byte counts per stream instead of splicing the marker into the captured text - files: crates/foundry-cli/src/runner.rs, crates/foundry-core/src/job.rs - run: cargo test
22. [x] The review TUI edit mode suspends the terminal and opens $EDITOR on the draft, resuming on save, replacing the append-only inline editor - files: crates/foundry-cli/src/main.rs - run: cargo test -p foundry-cli
